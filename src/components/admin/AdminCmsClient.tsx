...
export default function AdminCmsClient({ initialPages }: Props) {
  ...
  const refreshData = async () => {
    ...
    // TypeScript ne râlera plus ici car le Repository renvoie un PaginatedResponse
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-serif font-bold text-[#0F1F3D]">
            {selectedPage 
              ? t.editTitle.replace("{name}", selectedPage.titre_fr) 
              : t.title}
          </h1>
          <p className="text-slate-500 text-sm mt-0.5">
            {selectedPage ? t.editSubtitle : t.subtitle}
          </p>
        </div>
      </div>
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${page.est_publiee ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                  {page.est_publiee ? t.statusPublished : t.statusDraft}
                </span>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-50 flex justify-between items-center">
                <span className="text-[10px] text-slate-400 font-bold uppercase">
                  {t.blocksCount.replace("{count}", page.blocs.length.toString())}
                </span>
                <span className="text-sm font-bold text-[#C9A84C]">{t.btnEdit}</span>
              </div>
            </div>
          ))}
        </div>
