...
export default function AdminChatbotClient({ initialFaqs }: Props) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Supprimer cette question de la base de connaissances ?")) return;
    try {
  ...
  return (
    <div className="space-y-6 max-w-[1200px]">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-serif font-bold text-[#0F1F3D]">Intelligence du Chatbot (FAQ)</h1>
          <p className="text-slate-500 text-sm mt-0.5">Gérez les réponses automatiques de votre assistant virtuel.</p>
        </div>
          ...
          <div key={faq.id} className={`bg-white p-6 rounded-2xl border transition-all ${faq.est_actif ? 'border-slate-100 shadow-sm' : 'border-slate-100 opacity-60 grayscale'}`}>
            <div className="flex justify-between items-start gap-6">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-2 py-0.5 bg-salma-bg border border-salma-border rounded text-[9px] font-black uppercase text-salma-gold tracking-widest">
                    {faq.categorie || "Général"}
                  </span>
                  {!faq.est_actif && <span className="text-[9px] font-bold text-red-500 uppercase">Désactivé</span>}
                </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  {faq.est_actif ? "Désactiver" : "Activer"}
                </button>
                <button 
