...
//  Respect strict de l'architecture i18n SALMA
// ==============================================================================
...
interface Props {
  /** Titre de la section (doit être passé déjà localisé par le parent) */
  title: string;
  /** Description de la section (doit être passée déjà localisée par le parent) */
  description: string;
  /** Icône décorative */
...
export default function AdminComingSoon({ title, description, icon }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
      {/* Icône avec fond léger Navy */}
      <div className="w-16 h-16 rounded-2xl bg-[#0F1F3D]/5 flex items-center justify-center text-[#0F1F3D]/40 mb-6">
        {icon}
      </div>

      {/* Textes passés par le parent */}
      <h1 className="text-2xl font-serif font-bold text-[#0F1F3D] mb-3">
        {title}
      </h1>
      ...
      <div className="flex items-center gap-2 px-4 py-2 bg-[#C9A84C]/10 rounded-full border border-[#C9A84C]/30">
        ...
        <span className="text-xs font-semibold text-[#A68635]">
