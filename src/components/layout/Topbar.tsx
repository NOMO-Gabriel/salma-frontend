...
// =============================================================================
//  Hook : détecte la direction du scroll
// =============================================================================
...
export default function Topbar() {
  ...
  return (
    <div
      className={`
        w-full bg-salma-surface
        border-t-2 border-t-salma-gold/40
        border-b border-b-salma-border/15
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="hidden lg:flex items-center justify-between h-9 gap-4">
          ...
          {address && (
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <span className="text-[10px] font-medium tracking-wide text-salma-text-muted">{address}</span>
            </div>
          )}
          ...
          <div className="flex items-center gap-5">
            {phones && (
              <a
                href={`tel:${firstPhone}`}
                className="flex items-center gap-1.5 text-salma-primary hover:text-salma-gold transition-colors duration-150 group"
              >
                {Icon.phone}
            ...
            {SOCIAL_LINKS.map(({ label, href, letter }) => (
              <a
                ...
                className="
                  ...
                  text-[9px] font-bold text-salma-text-muted
        ...
        <div className="flex lg:hidden items-center justify-between h-8 gap-3">
          {phones && (
            <a
              href={`tel:${firstPhone}`}
              className="flex items-center gap-1.5 text-salma-primary hover:text-salma-gold transition-colors"
            >
              {Icon.phone}
          ...
          <a
            href={`https://wa.me/${firstPhone.replace("+", "")}`}
            target="_blank"
            rel="noopener noreferrer"
            className="
              flex items-center gap-1.5 px-3 py-1 rounded-full
              bg-green-500/15 text-green-400 border border-green-500/20
              hover:bg-green-500/25 transition-colors duration-150
            "
          >
            {Icon.whatsapp}
