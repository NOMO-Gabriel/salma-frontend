...
// =============================================================================
//  Section Témoignages — Version corrigée
//  Fix : dots de navigation mobile plus visibles avec indicateur "1/3"
//  Fix : support swipe tactile
//  Fix : meilleur responsive
// =============================================================================
...
export default function SuccessStories({ testimonials }: Props) {
  ...
  const goNext = useCallback(() => {
    goTo((activeIdx + 1) % displayed.length);
  }, [activeIdx, displayed.length, goTo]);

  const goPrev = useCallback(() => {
    goTo((activeIdx - 1 + displayed.length) % displayed.length);
  }, [activeIdx, displayed.length, goTo]);

  // 5. Swipe tactile (mobile)
  ...
  return (
    <section className="py-12 sm:py-16 lg:py-24 bg-gradient-to-b from-salma-surface/40 to-white dark:from-salma-surface/10 dark:to-salma-bg">
      <div className="container mx-auto px-4 sm:px-6">
        
        {/* Header Section */}
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          <SectionTitle title={texts.title} subtitle={texts.subtitle} align="center" />
          <p className="text-salma-text-muted text-xs sm:text-sm mt-3 sm:mt-4 max-w-xl mx-auto">
            {texts.description}
          </p>
        </div>
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="transition-all duration-300">
              <TestimonialCard 
                t={displayed[activeIdx]} 
                locale={locale} 
                verifiedLabel={texts.verified}
              />
            </div>

            {/* ── Navigation mobile : flèches + dots + compteur ────── */}
            <div className="flex items-center justify-center gap-4 mt-6">
              {/* Flèche gauche */}
              <button
                onClick={goPrev}
                className="w-9 h-9 rounded-full border border-salma-border bg-white dark:bg-salma-surface flex items-center justify-center text-salma-text-muted hover:text-salma-primary hover:border-salma-gold transition-colors shadow-sm"
                ...
                {displayed.map((_, i) => (
                  <button
                    ...
                    className={`rounded-full transition-all duration-300 ${
                      i === activeIdx 
                        ? "w-8 h-2.5 bg-salma-gold" 
                        : "w-2.5 h-2.5 bg-salma-border hover:bg-salma-gold/40"
                    }`}
            ...
            {/* Compteur textuel */}
            <p className="text-center text-[10px] text-salma-text-muted mt-2 tracking-wider">
              {activeIdx + 1} / {displayed.length}
            </p>
          </div>
        ...
        <div className="mt-10 sm:mt-12 lg:mt-16 flex justify-center">
          <div className="inline-flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-3 sm:py-4 bg-white dark:bg-salma-surface border border-salma-border rounded-2xl sm:rounded-3xl shadow-salma-sm">
            <div className="flex -space-x-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-white overflow-hidden bg-salma-bg">
                  {i % 2 === 0 ? <AvatarFemale /> : <AvatarMale />}
                </div>
              ))}
            </div>
            <div className="flex flex-col">
              <div className="flex gap-0.5 sm:gap-1 text-salma-gold">
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className="w-3 h-3 sm:w-4 sm:h-4 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                ))}
              </div>
              <p className="text-[10px] sm:text-[11px] font-bold text-salma-text-muted mt-1 uppercase tracking-tighter">
...
function TestimonialCard({ t, locale, verifiedLabel }: { t: TestimonialPublic, locale: string, verifiedLabel: string }) {
  ...
  return (
    <div className="h-full bg-white dark:bg-salma-surface border border-salma-border rounded-2xl sm:rounded-[2rem] p-6 sm:p-8 shadow-sm hover:shadow-salma-md transition-all duration-300 flex flex-col group">
      <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl overflow-hidden shadow-md relative flex-shrink-0">
          {photoUrl ? (
        ...
        <div className="min-w-0">
          <p className="font-bold text-sm sm:text-base text-salma-primary dark:text-white leading-tight">{t.nom_auteur}</p>
          <p className="text-[10px] text-salma-text-muted uppercase font-bold tracking-widest">{t.pays_auteur}</p>
        </div>
      </div>

      <blockquote className="text-xs sm:text-sm text-salma-text leading-relaxed italic flex-1 mb-4 sm:mb-6">
        {citation}
      </blockquote>

      <div className="pt-4 sm:pt-6 border-t border-salma-border flex items-center justify-between">
        <span className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[9px] font-black uppercase tracking-widest rounded-full">
