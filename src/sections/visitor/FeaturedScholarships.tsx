...
// =============================================================================
//  Correction Bug Hydratation : Utilisation du pattern 'mounted'
// =============================================================================
...
// ── Carte bourse ──────────────────────────────────────────────────────────────
function ScholarshipCard({ s, locale }: { s: ScholarshipPublicListItem; locale: "fr" | "en" }) {
  ...
  const deadlineStr = s.date_limite
    ? new Date(s.date_limite).toLocaleDateString("fr-FR", { day: "2-digit", month: "short", year: "numeric" })
    : null;
  ...
  return (
    ...
    >
      <div className="relative h-44 bg-salma-surface overflow-hidden flex-shrink-0">
        {imgUrl ? (
          <Image src={imgUrl} alt={title} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover group-hover:scale-105 transition-transform duration-500" />
        ) : (
        ...
        <div className="absolute top-3 left-3 flex gap-2 flex-wrap">
          ...
          {isClosed && <span className="px-2.5 py-1 text-[10px] font-bold bg-red-500 text-white rounded-full">{locale === "fr" ? "Fermé" : "Closed"}</span>}
          {isUrgent && !isClosed && <span className="px-2.5 py-1 text-[10px] font-bold bg-orange-500 text-white rounded-full animate-pulse">⚡ Urgent</span>}
        </div>
      </div>
      <div className="p-5 flex flex-col flex-1 gap-3">
        <span className="text-salma-gold text-[10px] font-bold uppercase tracking-widest">{locale === "fr" ? lvl.fr : lvl.en}</span>
        <h3 className="text-sm font-bold text-salma-primary leading-snug group-hover:text-salma-gold transition-colors line-clamp-2">{title}</h3>
        ...
        <div className="mt-auto pt-3 border-t border-salma-border flex items-center justify-between gap-2">
          <span className={`px-2.5 py-1 text-[10px] font-semibold rounded-full border ${cov.color}`}>{locale === "fr" ? cov.fr : cov.en}</span>
          {deadlineStr && <span className={`text-[10px] font-medium ${isUrgent ? "text-red-500 font-bold" : "text-salma-text-muted"}`}>{isUrgent ? "⚠ " : ""}Limite : {deadlineStr}</span>}
        </div>
      </div>
    </Link>
  );
...
export default function FeaturedScholarships({ scholarships }: Props) {
  ...
  const [texts, setTexts] = useState<{ title: string; subtitle: string; viewAll: string } | null>(null);
  ...
  // 3. Rendu d'attente (SSR & Hydratation)
  if (!mounted || !texts) {
    return (
  ...
  const data = IS_STATIC || scholarships.length === 0
  ...
  return (
    <section className="py-24 bg-white dark:bg-salma-bg">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {data.map((s) => (
            <ScholarshipCard key={s.id} s={s} locale={locale as "fr" | "en"} />
          ))}
        </div>
