...
export default function ScholarshipDetailClient({ scholarship, visibilityMap }: Props) {
  ...
  // 1. Chargement du contenu spécifique aux bourses
  useEffect(() => {
    cmsSwitcher.getScopeContent("bourses", locale).then((data) => {
      setContent(data as BoursesTexts);
    });
  }, [locale]);

  // 2. Tracking automatique de la vue
  useEffect(() => {
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          {isVisible('description_fr') && (
            <section className="bg-white dark:bg-salma-surface p-8 rounded-3xl border border-salma-border">
              ...
              <div className="text-salma-text-muted leading-relaxed">
                {desc}
              </div>
            </section>
          )}
          ...
          <div className="bg-salma-primary text-white p-8 rounded-3xl shadow-salma-lg sticky top-24">
            ...
            <div className="space-y-6 mb-10">
              {isVisible('date_limite') && scholarship.date_limite && (
                <div>
                  <span className="block text-[10px] uppercase tracking-widest text-salma-gold-light font-bold mb-1">{layoutDict.nav_contact.rdv_desc}</span>
            ...
            <div className="space-y-4">
              <a href={waLink} target="_blank" rel="noopener noreferrer" className="block w-full">
                <SalmaButton variant="gold" className="w-full py-4">{t.btnApply}</SalmaButton>
              </a>
              <Link href="/contact" className="block w-full">
                <SalmaButton variant="outline" className="w-full py-4 border-white text-white hover:bg-white hover:text-salma-primary">{t.btnForm}</SalmaButton>
              </Link>
            </div>
          </div>
        </div>
